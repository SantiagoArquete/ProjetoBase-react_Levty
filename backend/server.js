...
app.get('/api/perfil/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query('SELECT nome, curso, matricula, bio FROM perfil WHERE id = $1', [id]);
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT * FROM perfil WHERE matricula = $1 AND cpf = $2', [matricula, senha]);
    
    if (result.rows.length > 0) {
      res.json({ sucesso: true, mensagem: 'Login realizado com sucesso', usuario: result.rows[0] });
    } else {
